...
# services in LXC and KVM containers. Uses 2 machines with different
# series. Waits for the deployment then collects all logs and relevant
# info from the remote machines and destroys the environment.

# Call with an environment name, its default series and another series
# as arguments. e.g. ./test-lxc+kvm.sh maas-hw trusty precise
...
echo "Bootstrapping $JUJU_ENV with default-series: $SERIES1 using LXC and KVM containers"
...
juju deploy -e $JUJU_ENV cs:${SERIES2}/mysql --to kvm:0
